...
djng_forms_module.controller('FileUploadController', ['$scope', '$window', 'Upload', function($scope, $window, Upload) {
	this.uploadFiles = function(element, attrs, files) {
		var styles = $window.getComputedStyle(element[0]);
		var data = {width: styles.width, height: styles.height};
		...
		angular.forEach(files, function(file, index) {
			...
			Upload.upload({
				...
				element.css('background', field.url);
...
djng_forms_module.directive('ngfDrop', function() {
	return {
		restrict: 'AEC',
...
djng_forms_module.directive('ngfSelect', function() {
	return {
		restrict: 'AEC',
		controller: 'FileUploadController',
		link: function(scope, element, attrs, fileUploadController) {
