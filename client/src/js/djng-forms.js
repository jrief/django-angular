		...
		$http({
			...
			angular.forEach(self.digestUploadScope, function(scopeModel, formName) {
				if (response.data.hasOwnProperty(formName)) {
					djangoForm.setModels($scope[formName], response.data[formName].models);
				}
			});
...
djngModule.directive('djngEndpoint', function() {
	return {
		...
		controller: ['$scope', '$http', '$q', function($scope, $http, $q) {
			...
			// list of upload scope prefixes used by ng-models's inside this form
			this.digestUploadScope = {};

			this.uploadScope = function(method, extraData) {
				var deferred = $q.defer(), data = {};

				// merge the extra data with upload scope
				if (angular.isObject(extraData)) {
					angular.merge(data, extraData);
				}
				angular.forEach(self.digestUploadScope, function(scopeModel) {
					var values = $scope.$eval(scopeModel);
					if (values) {
						angular.merge(data, values);
					}
				});

				// submit data from the form to this endpoint on the server
				$http({
					url: self.endpointURL,
					method: method,
					data: data
...
djngModule.factory('djangoForm', function() {
	...
	return {
		...
		setModels: function(formCtrl, models) {
