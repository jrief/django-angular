			...
			this.uploadScope = function(deferred, method, extraData) {
				...
				angular.forEach(self.digestUploadScope, function(scopeModels) {
					...
					angular.forEach(scopeModels, function(scopeModel) {
						var value = $scope.$eval(scopeModel);
						if (value) {
							modelScopeData[scopeModel] = value;
...
djngModule.directive('ngModel', function($timeout) {
	...
	return {
		...
		require: ['^?djngFormsSet', '^?form'],
		link: function(scope, element, attrs, controllers) {
			var formsSetController = controllers[0], formController = controllers[1], digestUploadScope, formScope;

			if (!formsSetController)
				return;  // not for `ng-model`s outside <djng-forms-set>...</djng-forms-set>

			digestUploadScope = formsSetController.digestUploadScope;
			if (!angular.isArray(digestUploadScope[formController.$name])) {
				digestUploadScope[formController.$name] = [];
			}
			formScope = attrs.ngModel.match(varnameRegex);
			if (formScope) {
				digestUploadScope[formController.$name].push(formScope[0]);
			}
		}
	};
});
...
djngModule.directive('button', ['$q', '$window', function($q, $window) {
	...
	// are required, as in the case of the external form used by djangoshop-stripe.
		...
		link: function(scope, element, attrs, formsSetController) {
			...
			scope.reenableButton = function() {
				return function() {
					...
					angular.forEach(element.find('i'), function(icon) {
						icon = angular.element(icon);
						if (icon.attr('deactivated-class')) {
							icon.attr('class', icon.attr('deactivated-class'));
